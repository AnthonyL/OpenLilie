...
class ModuleENTControllerIntegrationTests {
	...
	void testSave(){
		...
		dc.springSecurityService = [
			...
			loggedIn: true,
			principal: loggedInUser]
		dc.params.title2="un titre";
		
		dc.save();
		assert dc.response.redirectedUrl.contains("/moduleENT/show")
		
		def  n = ModuleENT.list().size()
		dc.params.title2="un titre beaucoup mais beaucoup trop trop long !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!";
		dc.save();
	...
	void testUpdateExist(){
		def dc = new ModuleENTController();
		dc.params.title2 ="new title"
		dc.update(2, null);
		assert ModuleENT.get(2).title2 == "new title";
